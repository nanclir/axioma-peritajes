...
export default function Navbar() {
  ...
  return (
    <nav className="fixed w-full z-50 top-0 transition-all duration-300 glass border-b-0 border-slate-800">
      ...
      {isOpen && (
        <motion.div 
          ...
          className="md:hidden bg-navy-800 border-t border-slate-800"
        >
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {links.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="block px-3 py-2 rounded-md text-base font-medium text-slate-300 hover:text-gold-400 hover:bg-navy-700 transition-colors"
